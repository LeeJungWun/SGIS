...
<body>
		<div id="header"><a href="http://mslsgis.dothome.co.kr/index2.php" style="margin-left:auto;margin-right:auto;"><img src="/image/타이틀.jpg" border="0"></a></div>
		<div id="map"></div>
...
var showData;	//통계데이터 중 화면에 표출한 정보 파라미터명
var SGISurl;	//데이터 요청을 위한 url 주소
var UrlData;	//url 주소에서 옵션 설정
...
var checkcategory = function(){
 ...
 var checkBoxValue = [];
        ...
        for (var i = 0; i < checkboxs.length; i++) 
			...
			checkBoxValue += i;
			checkArray += checkboxs[i].value;
			//alert(checkboxs[i].value);
			if (box_count>=3) {
			//box_count++;
			checkboxs[i].checked = false;
			//checkboxs[checkBoxValue[i-1]].checked = false;
			//box_count = 0;
			//alert(checkArray);
          }
		  box_count++;
...
//	document.getElementById("menuSelectBar1").style.display="none";
	...
	//지도를 새로 그리기 위해 초기화를 한다.
	doClear();
/*
	if(Num == "사업체")
	{
		showData = "corp_cnt";
		SGISurl = "https://sgisapi.kostat.go.kr/OpenAPI3/stats/company.json";
		UrlData = {"accessToken" : accessToken, "adm_cd" : adm_cd, "year" : "2010", "low_search" : 1, "area_type" : "0", "class_deg" : "9", "bnd_year" : "2013"};
	}

	newdoReq(showData,SGISurl,UrlData)
...
//인구통계정보를 요청한다.
var newreqOpenApi = function(adm_cd, callback) {
	reqOpenAPIAuth("590a2718c58d41d9ae3b", "ab7fe94f9fb64336abd3", success);
	function success(accessToken) {
		$.ajax({
		type: "GET",
